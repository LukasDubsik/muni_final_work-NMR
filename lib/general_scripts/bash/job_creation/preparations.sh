...
EOF
	...
	if [[ $mcpb_step -ge 2 ]]; then
		...
		if [[ "$need_stage2" == "true" ]]; then
			...
			# Pre-patch Gaussian inputs in STAGE2_DIR so they are visible before submission
			# and match the scheduler resources (Gaussian reads these from Link0). 
			# MetaCentrum also recommends g16-prepare for this, but we want the files patched
			# in-place in STAGE2_DIR for inspection. :contentReference[oaicite:1]{index=1}
			local stage2_mem_gb=32
			local stage2_ncpus=16
			local stage2_scratch_gb=50
			...
			for com in \
				"$STAGE2_DIR/${name}_small_opt.com" \
				"$STAGE2_DIR/${name}_small_fc.com"  \
				"$STAGE2_DIR/${name}_large_mk.com"
			do
				[[ -f "$com" ]] || continue
				fix_link0_resources "$com" "$stage2_mem_gb" "$stage2_ncpus"
				fix_rwf_0mb "$com" "$stage2_scratch_gb"
				fix_au_basis "$com"
			done
